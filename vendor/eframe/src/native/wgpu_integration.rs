            ...
            && let Some(running) = &mut self.running
        {
                ...
                .and_then(|viewport| shared.viewports.get_mut(&viewport))
            {
                ...
                if let Some(egui_winit) = viewport.egui_winit.as_mut() {
                    ...
                    // Beautiful: MouseMotion used to RepaintNext every device tick,
                    // bypassing CursorMoved idle throttle (~6% CPU on canvas hover).
                    // Keep full-rate wake only while a button is down (stroke densify).
                    want_repaint = egui_winit.any_pointer_button_down();
                }
