...
pub struct State {
    ...
    /// Beautiful: last time idle (no button) cursor motion scheduled a repaint.
    /// Without this, CursorMoved at device Hz + Immediate present burns CPU on hover.
    last_idle_cursor_repaint: web_time::Instant,
}

impl State {
    /// True while any mouse/touch pointer button is held (Beautiful stroke densify).
    ...
    pub fn new(
        ...
        let mut slf = Self {
            ...
            last_idle_cursor_repaint: web_time::Instant::now()
    ...
    pub fn on_window_event(
        ...
        match event {
            ...
            WindowEvent::CursorMoved { position, .. } => {
                ...
                // Always update pointer in egui_input; only schedule a frame when needed.
                // Dragging / interacting stays full-rate for stroke latency.
                // Idle hover is capped (~30 Hz) so Immediate present doesn't run at mouse Hz.
                let dragging =
                    self.any_pointer_button_down || self.egui_ctx.is_using_pointer();
                let repaint = if dragging {
                    true
                } else {
                    let min = web_time::Duration::from_millis(33);
                    let now = web_time::Instant::now();
                    if now.duration_since(self.last_idle_cursor_repaint) >= min {
                        self.last_idle_cursor_repaint = now;
                        true
                    } else {
                        false
                    }
        ...
        if let Some(active_key) = logical_key.or(physical_key) {
            if pressed {
                if is_cut_command(self.egui_input.modifiers, active_key) {
                    ...
                    // Always emit Paste — even when clipboard has no text.
                    // Image-only clipboards (Win+Shift+S, Snipping Tool) used to
                    // return early with neither Paste nor Key::V, so Ctrl+V was silent.
                    let contents = self
                        .clipboard
                        .get()
                        .unwrap_or_default()
...
pub fn create_winit_window_attributes(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Required for wgpu DxgiFromVisual / DirectComposition transparency:
        // without WS_EX_NOREDIRECTIONBITMAP the DComp swapchain sits *behind*
        // an opaque window-sized redirection surface (the "white plate").
        // Must be set at CreateWindow — cannot be changed later.
